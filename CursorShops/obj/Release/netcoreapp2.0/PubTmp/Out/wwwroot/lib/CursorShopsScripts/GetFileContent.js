    ...
    if (IsMultimedia && IsMultimedia == true) {
        ...
        $.ajax({
            ...
            success: function (data, status, xhr) {    
                var blob = new Blob([convertDataURIToBinary(data.Content)], { type: 'application/octet-binary' }); 
                if (undefined === window.navigator.msSaveOrOpenBlob) {                                  
                    var link = document.createElement("a");
                    if ($.isNumeric(data.FileName.substring(0, 1))) {
                        link.download = 'IMG_' + data.FileName;
                    } else {
